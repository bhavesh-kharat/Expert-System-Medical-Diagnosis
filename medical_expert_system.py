...
def get_answer_yes_no(disease):
    answer = input(disease + ": ").lower()
    if answer in ["yes", "no"]:
        return answer
    else:
        print("Please provide answer in Yes/No only.")
        return get_answer_yes_no(disease)
...
class Greetings(KnowledgeEngine):
	...
	def _initial_action(self):
		print("")
		print("Hi! I am Dr.Bhavesh, I am here to help you make your health better.")
	...
	def symptom_0(self):
		answer = get_answer_yes_no("headache")
		self.declare(Fact(headache=answer))

	@Rule(Fact(action='find_disease'), NOT(Fact(back_pain=W())),salience = 1)
	def symptom_1(self):
		answer = get_answer_yes_no("back pain")
		self.declare(Fact(back_pain=answer))

	@Rule(Fact(action='find_disease'), NOT(Fact(chest_pain=W())),salience = 1)
	def symptom_2(self):
		answer = get_answer_yes_no("chest pain")
		self.declare(Fact(chest_pain=answer))

	@Rule(Fact(action='find_disease'), NOT(Fact(cough=W())),salience = 1)
	def symptom_3(self):
		answer = get_answer_yes_no("cough")
		self.declare(Fact(cough=answer))

	@Rule(Fact(action='find_disease'), NOT(Fact(fainting=W())),salience = 1)
	def symptom_4(self):
		answer = get_answer_yes_no("fainting")
		self.declare(Fact(fainting=answer))

	@Rule(Fact(action='find_disease'), NOT(Fact(fatigue=W())),salience = 1)
	def symptom_5(self):
		answer = get_answer_yes_no("fatigue")
		self.declare(Fact(fatigue=answer))
	 
	@Rule(Fact(action='find_disease'), NOT(Fact(sunken_eyes=W())),salience = 1)
	def symptom_6(self):
		answer = get_answer_yes_no("sunken eyes")
		self.declare(Fact(sunken_eyes=answer))
	
	@Rule(Fact(action='find_disease'), NOT(Fact(low_body_temp=W())),salience = 1)
	def symptom_7(self):
		answer = get_answer_yes_no("low body temperature")
		self.declare(Fact(low_body_temp=answer))
	
	@Rule(Fact(action='find_disease'), NOT(Fact(restlessness=W())),salience = 1)
	def symptom_8(self):
		answer = get_answer_yes_no("restlessness")
		self.declare(Fact(restlessness=answer))
	
	@Rule(Fact(action='find_disease'), NOT(Fact(sore_throat=W())),salience = 1)
	def symptom_9(self):
		answer = get_answer_yes_no("sore throat")
		self.declare(Fact(sore_throat=answer))
	
	@Rule(Fact(action='find_disease'), NOT(Fact(fever=W())),salience = 1)
	def symptom_10(self):
		answer = get_answer_yes_no("fever")
		self.declare(Fact(fever=answer))

	@Rule(Fact(action='find_disease'), NOT(Fact(nausea=W())),salience = 1)
	def symptom_11(self):
		answer = get_answer_yes_no("Nausea")
		self.declare(Fact(nausea=answer))

	@Rule(Fact(action='find_disease'), NOT(Fact(blurred_vision=W())),salience = 1)
	def symptom_12(self):
		answer = get_answer_yes_no("Blurred Vision")
		self.declare(Fact(blurred_vision=answer))
